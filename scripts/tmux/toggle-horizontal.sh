...
main() {
  ...
  if [ -n "$right_pane" ]; then
    ...
    tmux set-option -wu '@toggle_right_pane' >/dev/null 2>&1
    return
  fi
  ...
  if [ -n "$split_size" ]; then
    right_pane=$(tmux split-window -h -P -F '#{pane_id}' -t "$main_pane" -c "$pane_path" -p "$split_size")
  else
    right_pane=$(tmux split-window -h -P -F '#{pane_id}' -t "$main_pane" -c "$pane_path")
  fi
  tmux set-option -wq '@toggle_right_pane' "$right_pane"
}
